...
void initMemory() {
    ...
    if (myshm_ptr == MAP_FAILED) handle_error("mmap failed");
...
void closeMemory() {
    ...
    if (munmap(myshm_ptr, sizeof(struct shared_memory)) == -1) handle_error("munmap error");

    if (shm_unlink(SH_MEM_NAME) == -1) handle_error("shm_unlink error");

    if (close(fd_shm) == -1) handle_error("close error");
}
